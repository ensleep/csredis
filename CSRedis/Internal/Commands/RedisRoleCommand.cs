...
namespace CSRedis.Internal.Commands
{
    class RedisRoleCommand : RedisCommand<RedisRole>
    {
        ...
        static RedisMasterRole ParseMaster(int num, string role, RedisReader reader)
        {
            ...
            reader.ExpectType(RedisMessage.MultiBulk);
            var slaves = new Tuple<string, int, Int64>[reader.ReadInt(false)];
            for (int i = 0; i < slaves.Length; i++)
            {
                ...
                Int64 slave_offset = Int64.Parse(reader.ReadBulkString());
                slaves[i] = new Tuple<string, int, Int64>(ip, port, slave_offset);
            }
